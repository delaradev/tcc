"""Funções de loss para segmentação"""
import tensorflow as tf


def tversky_loss(alpha: float = 0.7, beta: float = 0.3, smooth: float = 1e-6):
    """
    Tversky loss para classes desbalanceadas
    alpha > beta penaliza mais falsos negativos (evita perder pivôs pequenos)
    """
...
def combined_loss(alpha: float = 0.5, beta: float = 0.5):
    """Combinação de Tversky e Binary Crossentropy"""
